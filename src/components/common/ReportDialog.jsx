...
function BootstrapDialogTitle(props) {
  const { children, onFullScreen, onClose, loading, onNewWindow, ...other } =
    props
...
export default function ReportDialog({ title, repType, repId, open, setOpen }) {
  ...
  const getReportToken = async () => {
    ...
    if (Api.status === 200) {
      setEmbedConfig({
        ...
        accessToken: reponse.azureToken,
      })
      setLoading(false)
    } else {
      setOpen(false)
  ...
  const getContent = async () => {
    if (repType === 'URL') await getReportUrl()
    else await getReportToken()
  ...
  const handleNewWindow = async () => {
    window.open(
      `/reports/${repType.toLowerCase()}/${repId}`,
      '_blank',
      `toolbar=no,
     location=no,
     status=no,
