...
const Action = ({ data, setData, handleOpenModal }) => {
  ...
  const handleResetPassword = async (usrId) => {
    ...
    const resetPasword = confirm(
      '¿Está seguro que desea restabler la contraseña del usuario?'
    )
    ...
    if (api.status === 200) {
      setSuccessMessage(
        api.message ? api.message : 'Usuario restablecido correctamente'
      )
  ...
  const handleDeleteUser = async (usrId) => {
    ...
    try {
      ...
      if (api.status === 200) {
        setSuccessMessage(
          api.message ? api.message : 'Usuario eliminado correctamente'
        )
        getUsers()
      } else {
